...
记忆系统不是单一的聊天记录表，而是多层数据管理架构：

- **代码索引 / RAG**：对用户选择的工作区进行扫描、切块和 embedding 存储，供语义搜索和 Agent 对话召回相关代码片段。
- **白盒记忆**：记忆条目包含类型、标签、强度、版本、TTL 和来源任务等字段，所有记忆可在设置面板中查看、编辑和筛选。
- **Dream 引擎**：自动定期清理过期记忆、合并高度相似条目、归档低强度旧数据。Dream 运行结果可见，支持一键回滚到上一版本。
- **经验教训**：从工具调用和任务结果中沉淀使用模式、失败信息和会话摘要。
- **项目知识**：记录项目相关约定、技术栈和常见错误。
- **Agent 操作日记**：记录 Agent 工具调用轨迹，用于调试和模式分析。

在“设置 → 记忆系统”中可以：

- 查看所有记忆条目（按类型、标签筛选，支持直接编辑和删除）。
- 选择工作区建立代码索引。
- 管理 Dream 维护间隔、触发条件等参数。
- 查看 Dream 历史记录并回滚。
- 查看检索轨迹和 Agent 操作日记。

索引数据写入 HermesX 的本机 SQLite 数据库，不会在项目目录里生成文件。
...
HermesX 对 Hermes Agent 的任务执行采用 SSE + 轮询双模：

- **SSE 流式执行**：任务通过 `/v1/runs` 提交后，默认走 SSE 事件流接收实时进度。
- **断线自动轮询**：SSE 意外断开（网络波动、Agent 重启）时自动切换到轮询模式，每 30 秒查询一次任务状态，最长持续 2 小时。
- **exitReason 追踪**：完整记录任务终止原因（sse_timeout → polling_completed / polling_failed / polling_cancelled），调试时可精准定位断点。
- **Managed Runs 持久化**：运行中的任务记录持久化存储，App 重启后可恢复状态。
- **任务清扫**：定期清理过期和已完成的 run 记录，避免数据堆积。

这一机制让 HermesX 从"只能等短任务"进化为"可以等待数十分钟的长任务"——用户不必守着屏幕等结果。
...
src/
  ...
  services/                   前端业务层
    ...
    context.ts                Context 管理与记忆版本化
    addOnlyMemory.ts          白盒记忆写入与检索
    memoryMaintenance.ts      Dream 引擎（记忆清理/合并）
    managedRuns.ts            Hermes Run 生命周期与轮询
    mcp/                      MCP 客户端与管理器
...
| everything-claude-code | 自调试、错误模式、验证闭环 | HermesX 内置 `verifier`、`selfHealing`、错误模式和相关检查能力。 |
...
HermesX 的记忆是多种数据的组合，不是单一字段：

- 持久化状态（`persistedState.ts`）：应用配置、记忆维护状态、Dream 历史。
- Context 版本化管理（`context.ts`）：记忆条目带版本号，写入时通过 Jaccard 相似度自动标记旧版为 superseded，支持滚回到任意 Dream 历史快照。
- Dream 引擎（`memoryMaintenance.ts`）：定期自动运行，支持过期清理、相似合并、低强度归档三种策略。可通过配置控制触发间隔、最小写入阈值和冷却时间。
- SQLite `memory.db`：代码索引、经验教训、会话摘要、项目知识、工具统计。
- 内存缓存：运行时索引、Agent 操作日记、任务结果缓存。
...
HermesX 不是"提交任务就死等 SSE 流"，而是对 Hermes Agent `/v1/runs` API 做了完整生命周期封装：

- **SSE + 轮询双模**：默认使用 SSE 流式接收事件，当连接意外断开（网络波动、Agent 重启等），自动切换为每 30 秒轮询 `GET /v1/runs/:id` 的状态模式。
- **阶段化 exitReason**：区分 SSE 阶段原因（`sse_timeout`、`sse_disconnected`）和轮询阶段原因（`polling_completed`、`polling_failed`、`polling_cancelled`），前者不会被后者覆盖，方便诊断真正的断线原因。
- **2 小时硬上限**：轮询最长持续 2 小时（`HERMES_RUN_POLL_MAX_DURATION_MS`），防止 Agent 服务端任务无限等待导致客户端轮询永不终止。
- **Managed Runs 持久化**：所有 run 记录（包括运行中、已完成、失败）写入持久化存储，支持 taskId fallback 去重，并定期清扫过期记录。

对应模块：

- `src/api/adapters/hermes.ts`：SSE 流、轮询循环、exitReason 追踪。
- `src/services/managedRuns.ts`：Run 记录的持久化、去重和清扫。
- `src/constants.ts`：超时、轮询间隔等常量配置。
...
这个模式适合调试前端界面。涉及 Tauri IPC、本地文件、桌面宠物窗口等能力时，应使用桌面开发环境。
...
```bash
npm run test          # 前端 TypeScript 测试（Vitest）
npm run build:web     # 前端构建检查
```

前端测试覆盖：工具注册、工具执行、路径审批、记忆写入、记忆维护、上下文管理等模块。
...
HermesX 启动后自动连接 `hermesX-admin` 版本管理平台检查新版本。设置中心可查看当前版本和更新日志，一键跳转下载最新安装包。
